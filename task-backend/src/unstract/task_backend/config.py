...
class BaseWorkerConfig(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="TASK_", env_file=".env", env_file_encoding="utf-8", extra="ignore"
    )

    # Worker Configuration
    backend_type: str = Field(default="celery", description="Backend type to use")
    worker_name: str = Field(
        default="", description="Worker instance name (auto-generated if empty)"
    )
    ...
    backend_health_timeout: int = Field(
        default=10, description="Backend health timeout in seconds"
    )
...
class HatchetWorkerConfig(BaseWorkerConfig):
    ...
    hatchet_token: str = Field(description="Hatchet client token (required)")
    hatchet_server_url: str = Field(
        default="https://app.hatchet.run", description="Hatchet server URL"
    )
    hatchet_tls_strategy: str = Field(default="tls", description="Hatchet TLS strategy")
    hatchet_worker_name: str = Field(
        default="task-worker", description="Hatchet worker name"
    )
...
class TemporalWorkerConfig(BaseWorkerConfig):
    ...
    temporal_task_queue: str = Field(
        default="task-queue", description="Temporal task queue"
    )
    temporal_worker_identity: str = Field(
        default="task-worker-01", description="Temporal worker identity"
    )
