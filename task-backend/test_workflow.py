...
def test_task_chaining():
    ...
    if hasattr(backend, 'app'):  # Celery backend
        ...
        result2 = celery_task.apply_async(args=["Result is ", number_result], queue='api_processing')
    ...
    if hasattr(backend, 'app'):
        try:
            ...
            workflow_chain = chain(
                ...
                format_task.s().set(queue='api_processing')  # Result (15) becomes first arg
            )
...
def test_workflow_patterns():
    ...
    ], name="sequential_test", description="Sequential add->format workflow")
    ...
    try:
        ...
        while True:
            ...
            print(f"   Status: {result.status}, Progress: {result.progress_percentage:.1f}%")
    ...
    parallel_workflow = WorkflowDefinition.parallel([
        ...
        ("add_numbers", {"a": 2, "b": 8})
    ], name="parallel_test", description="Parallel add operations")
    ...
    mixed_workflow = WorkflowDefinition.mixed([
        ...
        Parallel([  # Step 2: Multiple operations with result
    ...
    ], name="mixed_test", description="Mixed sequential->parallel->sequential workflow")
