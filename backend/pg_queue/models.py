...
class PgQueueMessage(models.Model):
    ...
    function, no ``CREATE EXTENSION`` (extension-free decision). The
    ...
    # Claim state (scan-past fix) — QueueMessageState {ready, claimed}.
...
class PgOrchestratorLock(models.Model):
    ...
    **transaction-pooled PgBouncer** (the queue's connection path),
...
class PgBatchDedup(models.Model):
    ...
    DB-side function, extension-free — same posture as the siblings.
    """
...
class PgBarrierState(models.Model):
    ...
    same posture as ``PgQueueMessage``.
    """
    ...
    # Progress liveness: re-stamped to now() on enqueue AND on every
...
class PgPeriodicSchedule(models.Model):
    """Inert mirror of a scheduled pipeline's cron definition.
    ...
    # that's the ramp control, which does not exist yet. Until it lands,
