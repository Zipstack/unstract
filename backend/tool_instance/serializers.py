...
class ToolInstanceSerializer(AuditSerializer):
    ...
    def create(self, validated_data: dict[str, Any]) -> Any:
        ...
        if TIKey.STEP not in validated_data:
            validated_data[TIKey.STEP] = workflow.workflow_tool.count() + 1
...
class ToolInstanceReorderSerializer(Serializer):
    ...
    def validate(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        tool_instance_count = workflow.workflow_tool.count()
        ...
        existing_tool_instance_ids = workflow.workflow_tool.values_list("id", flat=True)
