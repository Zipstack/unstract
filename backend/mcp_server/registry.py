    ...
    registry.register(
        MCPTool(
            ...
            description=(
                ...
                "Documents are supplied as HTTPS URLs on an AWS S3 host, which "
                "Unstract fetches server-side; links to any other host are "
                "rejected, so upload to S3 first if the document is not "
                "already there. Use a pre-signed URL for a private object — "
                "the usual case. Extraction is asynchronous: when it does not "
