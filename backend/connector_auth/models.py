...
from connector_auth.pipeline.google import GoogleAuthHelper
...
class ConnectorAuth(AbstractUserSocialAuth):
    ...
    def get_and_refresh_tokens(
        ...
        if new_access_token != existing_access_token:
            ...
            for connector_instance in related_connector_instances:
                ...
                logger.info(
                    "Refreshed access token for connector "
                    f"{connector_instance.id}, "
    ...
    def check_credential_format(
        ...
        if (
            ...
            if raise_exception:
                raise ValueError(
                    "Auth credential should have provider,"
                    " uid and connector guid"
                )
            return False
