...
from migration_tools.unstract_migrations import UnstractMigration
...
class DataMigrator:
    ...
    def connect_db(self, db_config) -> connection:
        return psycopg2.connect(**db_config)

    def create_tracking_table_if_not_exists(self):
        conn = self.connect_db(self.dest_db_config)
    ...
    def fetch_schema_names(
        self, schemas_to_migrate: Optional[list[str]] = None
    ) -> list[tuple[int, str]]:
        conn = self.connect_db(self.src_db_config)
        cur = conn.cursor()

        # Process schemas_to_migrate: trim spaces and remove empty entries
        if schemas_to_migrate:
            schemas_to_migrate = [
    ...
    def migrate_single_row(
        self,
        row: tuple[Any, ...],
        dest_cursor: cursor,
        column_names: list[str],
        new_key_transaction: dict[str, dict[str, Any]],
    ) -> tuple[Any, ...]:
        row = list(row)

        for key, transaction in new_key_transaction.items():
    ...
    def migrate_rows(
        ...
        while True:
            ...
            converted_rows = []
            for row in rows:
                converted_row = self.migrate_single_row(
                    row, dest_cursor, column_names, new_key_transaction
                )
    ...
    def adjust_auto_increment(
        ...
        try:
            ...
            if result:
                ...
                if column_type in ("bigint", "integer", "serial"):
                    max_id_query = (
    ...
    def migrate(
        ...
        self.create_tracking_table_if_not_exists()

        src_conn = self.connect_db(self.src_db_config)
        src_cursor: cursor = src_conn.cursor()

        dest_conn = self.connect_db(self.dest_db_config)
        dest_cursor: cursor = dest_conn.cursor()
        for migration in migrations:
            ...
            new_key_transaction = migration.get("new_key_transaction", {})
            ...
            if dest_table and "id" in column_names:
                self.adjust_auto_increment(dest_cursor, dest_table, dest_conn)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not schemas_to_migrate:
            confirmation = (
                input(
                    "No specific schemas provided. This will migrate all schemas. "
            ...
            if confirmation != "y":
                logger.info("Migration aborted by the user.")
                return
        else:
            schemas_to_migrate = schemas_to_migrate.split(",")

        # Organization Data (Schema)
        schema_names = migrator.fetch_schema_names(
