...
class RedisQueue(UnstractQueue):
    ...
    def dequeue(self, queue_name: str, timeout: int = 5) -> Any:
        ...
        Args:
            queue_name (str): The name of the queue.
            timeout (int): The timeout in seconds for the brpop operation. Default is 5 seconds.
        ...
        try:
            with RedisConnectionManager(self.get_engine) as conn:
                ...
                if message is None:
                    logger.debug(
                        f"No message received from queue '{queue_name}' within {timeout} seconds.", stack_info=True, exc_info=True
                    )
                    return None
                return message[1]
        except Exception as e:
    ...
    def peek(self, queue_name: str) -> Any:
        ...
        try:
            with RedisConnectionManager(self.get_engine) as conn:
                message = conn.lindex(queue_name, 0)  # Get the first item without removing it
                return message
        except Exception as e:
    ...
    def lindex(self, queue_name: str, index: int) -> Any:
        ...
        Returns:
            Any: The value at the specified index in the queue, or None if the index is out of range.
        ...
        try:
            ...
            raise ConnectorError(f"Error retrieving value from queue at index {index}: {e}") from e
    ...
    def keys(self, pattern: str = '*') -> List[str]:
        ...
        try:
            ...
            raise ConnectorError(f"Error retrieving keys with pattern '{pattern}': {e}") from e
