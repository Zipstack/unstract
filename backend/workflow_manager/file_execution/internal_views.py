...
class FileExecutionInternalViewSet(viewsets.ModelViewSet):
    ...
    def get_object(self):
        ...
        This avoids the expensive filtered queryset (3-table JOIN + query params)
        for single-object operations (retrieve, update, destroy).
        """
        pk = self.kwargs.get(self.lookup_field)
        queryset = WorkflowFileExecution.objects.filter(id=pk)
    ...
    def get_queryset(self):
        ...
        Only used for list() — single-object lookups go through get_object().
        """
        ...
        # Use file_hash OR provider_file_uuid (not both) to match manager logic
        if file_hash:
