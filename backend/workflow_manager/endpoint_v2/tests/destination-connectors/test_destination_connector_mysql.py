...
class TestDestinationConnectorMySQL(TestCase):
    ...
    def setUp(self) -> None:
        """Set up test data and real MySQL configuration."""
        ...
        if missing_vars:
            self.skipTest(
                f"Required MySQL environment variables not set: {', '.join(missing_vars)}"
            )
        ...
        # Expected columns based on configuration (MySQL data types)
        self.expected_columns = {
            ...
            "status": "enum",
            "user_field_1": "boolean",  # BOOLEAN maps to TINYINT in MySQL
            "user_field_2": "bigint",
    ...
    def verify_table_columns(self, table_name: str) -> None:
        ...
        for expected_column, expected_type in required_columns.items():
            ...
            # MySQL may return slightly different type names, so we do flexible matching
    ...
    def create_real_connector_instance(self) -> Mock:
        """Create connector instance using real MySQL connector."""
        connector = Mock()
        connector.connector_id = MySQL.get_id()
        connector.connector_metadata = self.mysql_config
        # Store reference to real connector for potential future use
        connector._real_mysql_connector = self.mysql_connector
        return connector
    ...
    def test_insert_into_db_happy_path_mysql(self) -> None:
        """Test successful insertion into real MySQL database."""
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                # This will execute real database operations using MySQL connector
        ...
        print(
            f"✅ Successfully inserted test data into MySQL table: {self.test_table_name}"
        )

    def test_insert_into_db_with_error_mysql(self) -> None:
        """Test insertion with error parameter into real MySQL database."""
        # Create mock objects
        mock_workflow = self.create_mock_workflow()
        mock_workflow_log = self.create_mock_workflow_log()
        mock_connector_instance = self.create_real_connector_instance()
        ...
        print(
            f"✅ Successfully inserted error data into MySQL table: {self.test_table_name}"
        )

    def test_mysql_connector_connection(self) -> None:
        """Test that the MySQL connector can establish a connection."""
        # Test the real MySQL connector directly
        try:
            engine = self.mysql_connector.get_engine()
            self.assertIsNotNone(engine)
            print("✅ MySQL connector successfully established connection")
            ...
            cursor.close()
            print("✅ MySQL connector successfully executed test query")

            # Clean up
            if hasattr(engine, "close"):
                engine.close()
        except Exception as e:
            self.fail(f"MySQL connector failed to connect: {str(e)}")
    ...
    def test_mysql_specific_data_types(self) -> None:
        """Test MySQL-specific data types and constraints."""
        ...
        try:
            ...
            create_table_query = self.mysql_connector.get_create_table_base_query(
            ...
            cursor.close()
            print("✅ MySQL ENUM data type verification successful")
    ...
    def _verify_migration_success(self, table_name: str) -> None:
        ...
        for column_name, expected_type in expected_v2_columns.items():
            ...
            if expected_type == "json":
                # MySQL might return different representations for JSON
                self.assertIn(
                    actual_type,
    ...
    def tearDown(self) -> None:
        ...
        #     cursor.execute(f"DROP TABLE IF EXISTS LEGACY_OUTPUT_MYSQL")
        #     cursor.close()
        #     engine.close()
        # except Exception:
