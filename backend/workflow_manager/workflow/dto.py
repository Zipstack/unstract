...
class ExecutionResponse:
    ...
    def remove_result_metadata_keys(self, keys_to_remove: List[str] = []) -> None:
        """
        Removes specified keys from the 'metadata' dictionary within each 'result' dictionary 
        in the 'result' list attribute of the instance. If 'keys_to_remove' is empty, the 
        'metadata' key itself is removed.
    ...
    def _remove_specific_keys(self, result: dict, keys_to_remove: List[str]) -> None:
        """
        Removes specified keys from the 'metadata' dictionary within the provided 'result' dictionary. 
        If 'keys_to_remove' is empty, the 'metadata' dictionary is cleared.
    ...
    def _update_metadata(self, result: dict, metadata: dict) -> None:
        """
        Updates the 'metadata' key in the provided 'result' dictionary. If 'metadata' is empty, 
        removes the 'metadata' key from 'result'.

        Args:
            result (dict): The dictionary to be updated.
            metadata (dict): The new metadata dictionary to be set. If empty, 'metadata' is removed.
        """
        if metadata:
