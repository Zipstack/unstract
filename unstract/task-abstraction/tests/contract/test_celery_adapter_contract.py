...
class TestCeleryBackendAdapterContract:
    ...
    def sample_workflow_definition(self) -> WorkflowDefinition:
        ...
        return WorkflowDefinition(
            ...
            tasks=[
                TaskDefinition(
                    config=TaskConfig(name="preprocess-document", timeout_minutes=5),
                    function_name="preprocess_document",
                    parents=[],
                ),
                TaskDefinition(
                    config=TaskConfig(name="extract-content", timeout_minutes=10),
                    function_name="extract_content",
                    parents=["preprocess-document"],
                ),
                TaskDefinition(
                    config=TaskConfig(name="post-process", timeout_minutes=3),
                    function_name="post_process",
                    parents=["extract-content"],
                ),
                TaskDefinition(
                    config=TaskConfig(name="generate-summary", timeout_minutes=5),
    ...
    async def test_workflow_execution_async(
        ...
        workflow_id = await celery_client.run_workflow_async(
            "test-celery-workflow", input_data
        )
    ...
    async def test_task_result_retrieval(
        ...
        task_result = await celery_client.get_task_result(
            workflow_id, "preprocess-document"
        )
    ...
    async def test_connection_failure_handling(self):
        ...
        invalid_client = CeleryTaskClient(
            broker_url="redis://invalid:9999/0", result_backend="redis://invalid:9999/0"
        )
    ...
    async def test_concurrent_workflow_execution(
        ...
        for i in range(5):
            task = celery_client.run_workflow_async(
                "test-celery-workflow", {"test": f"data_{i}"}
            )
            tasks.append(task)
    ...
    async def test_celery_task_routing(
        ...
        registration_result = await celery_client.register_workflow(workflow_with_routing)
    ...
    async def test_celery_retry_policy(
        ...
        registration_result = await celery_client.register_workflow(workflow_with_retry)

        assert registration_result.success

    @pytest.mark.asyncio
    async def test_celery_chord_pattern(self, celery_client: CeleryTaskClient):
        ...
        workflow_id = await celery_client.run_workflow_async(
            "test-chord-workflow", {"parallel_count": 3}
        )
