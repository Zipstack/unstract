...
class TestCircuitBreakerIntegration:
        ...
        return [
            ...
            CircuitBreakerTestCase(
                ...
                failure_sequence=[
                    ...
                    False,
                    True,
                ],  # 2 failures, then success after timeout
                expected_state_transitions=[
                    CircuitState.CLOSED,
                    CircuitState.OPEN,
            ...
            CircuitBreakerTestCase(
                ...
                failure_sequence=[
                    ...
                    False,
                    False,
                ],  # Failures, then more failure in half-open
                expected_state_transitions=[
                    CircuitState.CLOSED,
                    CircuitState.OPEN,
    ...
    async def test_circuit_breaker_state_transitions(
        self, circuit_breaker_test_cases: list[CircuitBreakerTestCase]
    ):
        ...
        for test_case in circuit_breaker_test_cases:
            ...
            expected_transitions = [
                CircuitState.CLOSED
            ] + test_case.expected_state_transitions
            assert (
                len(state_history) == len(expected_transitions)
            ), f"Test case '{test_case.name}': expected {len(expected_transitions)} states, got {len(state_history)}"

            for i, (expected, actual) in enumerate(
                zip(expected_transitions, state_history, strict=False)
            ):
                assert (
                    actual == expected
    ...
    async def test_circuit_breaker_recovery(self, migration_circuit_breaker):
        ...
        for i in range(3):
            await migration_circuit_breaker.call(
                initially_failing_operation, fallback_operation
            )
    ...
    async def test_circuit_breaker_with_backend_fallback(self):
        ...
        backend_cb = BackendCircuitBreaker(
            backend_name="hatchet", failure_threshold=2, recovery_timeout=1.0
        )
    ...
    async def test_circuit_breaker_metrics_collection(self):
        ...
        from unstract.task_abstraction.circuit_breaker import (
            MetricsCollectingCircuitBreaker,
        )

        metrics_cb = MetricsCollectingCircuitBreaker(
            name="test_circuit", failure_threshold=2, recovery_timeout=1.0
        )
    ...
    @pytest.mark.asyncio
    async def test_circuit_breaker_with_different_error_types(
        self, migration_circuit_breaker
    ):
        ...
        for i in range(len(error_types) + 2):  # More than threshold
            result = await migration_circuit_breaker.call(
                multi_error_operation, error_handling_fallback
            )
        ...
        for i in range(5):
            try:
                result = await custom_cb.call(
                    selective_failing_operation, selective_fallback
                )
            except ValueError:
    ...
    def test_circuit_breaker_interface_compliance(self):
        ...
        required_methods = ["call", "state", "reset", "get_metrics"]
