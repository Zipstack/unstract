...
from unstract.task_abstraction.interfaces import TaskClient, WorkflowResult, TaskResult
from unstract.task_abstraction.models import WorkflowDefinition, WorkflowConfig, TaskDefinition, TaskConfig
...
class TestCrossBackendCompatibilityIntegration:
    ...
    async def test_workflow_definition_compatibility(
        ...
        for test_case in cross_backend_test_cases:
            ...
            assert source_registration.success, f"Failed to register on {test_case.source_backend}"
            
            # Register same workflow on target backend
            target_registration = await target_client.register_workflow(
                test_case.workflow_definition
            )
            assert target_registration.success, f"Failed to register on {test_case.target_backend}"
            ...
            for i, (source_task, target_task) in enumerate(zip(source_definition.tasks, target_definition.tasks)):
                assert source_task.config.name == target_task.config.name, \
    ...
    async def test_data_serialization_compatibility(
        ...
        for test_case in cross_backend_test_cases:
            ...
            for i, test_data in enumerate(test_data_variations):
                ...
                # Compare results
                source_result = await source_client.get_workflow_result(source_workflow_id)
                target_result = await target_client.get_workflow_result(target_workflow_id)
                
                # Results should have same structure (may have different backend-specific metadata)
                assert source_result.status == target_result.status, \
                    f"Status mismatch for data variant {i}: {source_result.status} != {target_result.status}"
                
                # Task results should be compatible
                assert len(source_result.task_results) == len(target_result.task_results), \
            ...
            for backend_type, client in backend_clients.items():
                ...
                try:
                    workflow_id = await client.run_workflow_async(
                        scenario["workflow_override"].config.name,
                        scenario["input_data"]
                    )
                    ...
                    assert scenario["expected_error_type"] in str(e) or scenario["expected_error_type"] == error_type_name, \
    ...
    async def test_result_format_consistency(
        ...
        for i, other_result in enumerate(result_list[1:], 1):
            ...
            required_fields = ['workflow_id', 'status', 'task_results', 'created_at', 'completed_at']
            for field in required_fields:
                assert hasattr(other_result, field), \
                    f"Backend {backend_name} missing required field: {field}"
                assert hasattr(base_result, field), \
                    f"Base backend missing required field: {field}"
            
            # Status should be comparable
            assert other_result.status == base_result.status, \
                f"Status mismatch between backends: {other_result.status} != {base_result.status}"
            
            # Task results should have same structure
            assert len(other_result.task_results) == len(base_result.task_results), \
    ...
    async def test_backend_migration_data_transfer(
        ...
        assert migration_result.target_workflow_id is not None, "Target workflow ID should be set"
        
        # Verify target workflow continues execution
        target_client = backend_clients[BackendType.HATCHET]
        target_result = await target_client.get_workflow_result(migration_result.target_workflow_id)
    ...
    async def test_concurrent_multi_backend_execution(
        ...
        assert completed_count >= expected_completions, \
    ...
    async def test_backend_specific_feature_compatibility(
        ...
        for backend_type, config in backend_features.items():
            ...
            assert registration_result.success, f"Failed to register {backend_type} specific workflow"
            
            # Execute workflow
            workflow_id = await client.run_workflow_async(
                config["test_workflow"].config.name,
                {"test_features": config["features"]}
            )
    ...
    def test_cross_backend_interface_compliance(self):
        ...
        for backend_class in backend_classes:
            assert issubclass(backend_class, TaskClient), \
