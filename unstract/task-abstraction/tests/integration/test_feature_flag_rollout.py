...
class TestFeatureFlagRolloutIntegration:
    ...
    async def test_percentage_based_rollout(
        ...
        rollout_test_cases: List[RolloutTestCase]
    ):
        """Test percentage-based rollout distribution."""
        
        percentage_cases = [
            case for case in rollout_test_cases 
        ...
        for test_case in percentage_cases:
            ...
            with patch('unstract.flags.feature_flag.check_feature_flag_status') as mock_flag:
                ...
                assert min_expected <= enabled_count <= max_expected, \
    ...
    async def test_canary_rollout(
        ...
        canary_cases = [
            case for case in rollout_test_cases 
            if case.strategy == RolloutStrategy.CANARY
        ]
    ...
    async def test_gradual_rollout_progression(self, feature_flag_manager):
        ...
        for stage in rollout_stages:
            with patch('unstract.flags.feature_flag.check_feature_flag_status') as mock_flag:
                ...
                def mock_progressive_rollout(flag_key, namespace, entity_id, context=None):
                    if flag_key == "task_abstraction_enabled":
                        hash_value = int(hashlib.md5(entity_id.encode()).hexdigest(), 16)
                        user_bucket = hash_value % 100
    ...
    async def test_rollback_scenario(self, feature_flag_manager):
        ...
        test_users = [{"user_id": f"user_{i}", "organization_id": "test_org"} for i in range(100)]
        ...
        assert enabled_users_25.issubset(enabled_users_75), \
            "Rollback users should be subset of original rollout users"
        
        # Verify approximate counts
        assert 70 <= len(enabled_users_75) <= 80, f"75% rollout: expected ~75 users, got {len(enabled_users_75)}"
        assert 20 <= len(enabled_users_25) <= 30, f"25% rollout: expected ~25 users, got {len(enabled_users_25)}"
    ...
    async def test_multi_flag_rollout_consistency(self, feature_flag_manager):
        ...
        test_users = [{"user_id": f"user_{i}", "organization_id": "test_org"} for i in range(50)]
        ...
        with patch('unstract.flags.feature_flag.check_feature_flag_status') as mock_flag:
            def mock_dependent_rollout(flag_key, namespace, entity_id, context=None):
                ...
                if flag_key == "task_abstraction_enabled":
                    ...
                    return user_bucket < 25 and user_bucket < 50  # 25% rollout, subset of task_abstraction
            ...
            for user_context in test_users:
                ...
                for flag in related_flags:
                    is_enabled = await feature_flag_manager.is_feature_enabled(flag, user_context)
                    if is_enabled:
            ...
            assert hatchet_users.issubset(task_abstraction_users), \
                "Hatchet users should be subset of task_abstraction users"
            
            # Verify approximate counts
            assert 20 <= len(task_abstraction_users) <= 30, f"Task abstraction: expected ~25 users, got {len(task_abstraction_users)}"
            assert 10 <= len(hatchet_users) <= 15, f"Hatchet: expected ~12 users, got {len(hatchet_users)}"
    ...
    async def test_concurrent_rollout_evaluation(self, feature_flag_manager):
        ...
        with patch('unstract.flags.feature_flag.check_feature_flag_status') as mock_flag:
            ...
            tasks = [
                feature_flag_manager.is_feature_enabled("task_abstraction_enabled", context)
            ...
            for i, result in enumerate(results):
                ...
                if user_id not in user_results:
                    ...
                    assert user_results[user_id] == result, \
    ...
    async def test_rollout_with_context_variables(self, feature_flag_manager):
        ...
        context_scenarios = [
            {
                "context": {"user_id": "user_1", "organization_id": "org_1", "region": "us-west"},
                "expected_enabled": True  # US region gets early access
            },
            {
                "context": {"user_id": "user_2", "organization_id": "org_2", "region": "eu-central"},
                "expected_enabled": False  # EU region waits
            },
            {
                "context": {"user_id": "user_3", "organization_id": "org_3", "plan": "enterprise"},
                "expected_enabled": True  # Enterprise plan gets early access
            },
            {
                "context": {"user_id": "user_4", "organization_id": "org_4", "plan": "basic"},
        ...
        with patch('unstract.flags.feature_flag.check_feature_flag_status') as mock_flag:
            ...
            for scenario in context_scenarios:
                ...
                assert is_enabled == scenario["expected_enabled"], \
    ...
    def test_feature_flag_manager_interface_compliance(self):
        ...
        required_methods = [
            'is_feature_enabled', 'get_rollout_status', 'update_rollout_percentage'
        ]
