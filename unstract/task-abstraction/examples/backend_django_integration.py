...
from unstract.task_abstraction import BaseWorkflow, TaskContext, task, workflow
...
class UserOnboardingWorkflow(BaseWorkflow):
    ...
    def create_user_account(self, input_data: dict, ctx: TaskContext) -> dict:
        ...
        return {"user_id": user.id, "created": created, "username": user.username}
...
class PipelineDeploymentWorkflow(BaseWorkflow):
    ...
    def deploy_to_runner(self, input_data: dict, ctx: TaskContext) -> dict:
        ...
        if validation_data["validation_status"] != "passed":
            return {"deployment_status": "failed", "reason": "Pipeline validation failed"}
...
class WorkflowManagerConfig(AppConfig):
    ...
    def _register_workflows(self):
        ...
        try:
            ...
            logger.info(
                "No event loop available, workflows will be registered on first use"
            )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--register", action="store_true", help="Register all backend workflows"
        )
        parser.add_argument("--test", action="store_true", help="Test workflow execution")
        parser.add_argument(
            "--workflow", type=str, help="Specific workflow name for testing"
        )
        parser.add_argument(
            "--status", action="store_true", help="Show workflow registration status"
        )
        parser.add_argument("--backend", type=str, help="Task queue backend override")
    ...
    async def _async_handle(self, options):
        ...
        if options["register"]:
            ...
            summary = await register_service_workflows(
                service_name="django-backend", workflow_packages=["backend.workflows"]
            )
            self.stdout.write(
                self.style.SUCCESS(
        ...
        return JsonResponse(
            {
                ...
                "user_created": result.task_results.get("create-user-account", {}).get(
                    "result", {}
                ),
            }
        )
