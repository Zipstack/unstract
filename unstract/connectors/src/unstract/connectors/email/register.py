"""Email connector registration module."""
...
def register_connectors(connectors: dict[str, Any]) -> None:
    """Register all email connectors.

    Args:
        connectors: Dictionary to store registered connectors.
    """
    current_directory = os.path.dirname(os.path.abspath(__file__))
    package = "unstract.connectors.email"
        ...
        if os.path.isdir(connector_path) and not connector.startswith("__"):
            try:
                ...
                if metadata.get("is_active", False):
                    connector_class: UnstractEmail = metadata[Common.CONNECTOR]
    ...
    if len(connectors) == 0:
        logger.warning("No email connector found.")
