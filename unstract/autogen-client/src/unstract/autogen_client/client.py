...
class UnstractAutoGenClient(ChatCompletionClient):
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if self._enable_retries:
                ...
                stream_response = await self._execute_streaming_completion(completion_params)
    ...
    def count_tokens(self, messages: List[Dict[str, str]]) -> int:
        ...
        try:
            ...
            logger.warning(f"Token counting failed: {str(e)}, using fallback")
    ...
    def remaining_tokens(self) -> int:
        ...
        used_tokens = self._last_usage.prompt_tokens + self._last_usage.completion_tokens
    ...
    async def _execute_streaming_completion(self, params: Dict[str, Any]) -> Any:
        ...
        stream_response = await asyncio.get_event_loop().run_in_executor(None, _stream_call)
        ...
        for attempt in range(self._max_retries + 1):
            try:
                ...
                if attempt < self._max_retries:
                    ...
                    logger.error(f"All {self._max_retries + 1} attempts failed: {str(e)}")
                    break
    ...
    def _handle_exception(self, error: Exception) -> UnstractAutoGenError:
        ...
        if "timeout" in error_message or "timed out" in error_message:
            return UnstractTimeoutError(
                "Request timed out",
                original_error=error
            )
        
        # Connection errors
        if any(term in error_message for term in ["connection", "network", "dns", "unreachable"]):
            return UnstractConnectionError(
                "Failed to connect to Unstract adapter",
                original_error=error
            )
        ...
        return UnstractCompletionError(
            f"Unstract adapter completion failed: {error}",
            original_error=error
        )
