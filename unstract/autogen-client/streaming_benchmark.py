...
class StreamingBenchmarkAdapter:
    ...
    def completion(self, messages: list, stream: bool = False, **kwargs: Any) -> Any:
        ...
        response_func = self.response_templates.get(self.response_length, self._generate_medium_response)
...
class StreamingBenchmark:
    ...
    async def run_benchmark_suite(self) -> List[BenchmarkResult]:
        ...
        print("=" * 70)
        print("Testing streaming vs non-streaming performance across different response lengths")
        print()
    ...
    async def _benchmark_completion(
        ...
        messages = [
            ...
            UserMessage(content=f"Generate a {response_length} response for benchmarking.", source="user")
        ]
        ...
        try:
            if streaming:
                ...
                first_token_time = time.time()  # For non-streaming, first token is immediate
        ...
        first_token_latency = (first_token_time - start_time) if first_token_time else duration
    ...
    def _print_comparison(self, non_streaming: BenchmarkResult, streaming: BenchmarkResult):
        ...
        if non_streaming.duration > 0:
            duration_improvement = ((non_streaming.duration - streaming.duration) / non_streaming.duration) * 100
            throughput_improvement = ((streaming.tokens_per_second - non_streaming.tokens_per_second) / non_streaming.tokens_per_second) * 100
            first_token_improvement = ((non_streaming.first_token_latency - streaming.first_token_latency) / non_streaming.first_token_latency) * 100
            
            print(f"  📊 Streaming Benefits:")
    ...
    def _print_summary(self):
        ...
        if streaming_results and non_streaming_results:
            avg_streaming_tps = statistics.mean([r.tokens_per_second for r in streaming_results])
            avg_non_streaming_tps = statistics.mean([r.tokens_per_second for r in non_streaming_results])
            avg_streaming_latency = statistics.mean([r.first_token_latency for r in streaming_results])
            avg_non_streaming_latency = statistics.mean([r.first_token_latency for r in non_streaming_results])
            
            print(f"📈 Average Performance:")
            print(f"  Non-Streaming: {avg_non_streaming_tps:.1f} tokens/sec, {avg_non_streaming_latency:.3f}s first token")
            print(f"  Streaming: {avg_streaming_tps:.1f} tokens/sec, {avg_streaming_latency:.3f}s first token")
            
            overall_throughput_improvement = ((avg_streaming_tps - avg_non_streaming_tps) / avg_non_streaming_tps) * 100
            overall_latency_improvement = ((avg_non_streaming_latency - avg_streaming_latency) / avg_non_streaming_latency) * 100
            
            print(f"\n🎯 Overall Streaming Benefits:")
            print(f"  ⚡ Throughput improvement: +{overall_throughput_improvement:.1f}%")
            print(f"  🚀 First token latency improvement: +{overall_latency_improvement:.1f}%")
        ...
        print(f"  📝 Use streaming for responses > 100 tokens for better user experience")
        print(f"  ⚡ Use non-streaming for short responses < 50 tokens")
        print(f"  🔄 Consider caching for frequently requested content")
