...
def extract_pages(input_file: str, output_file: str, page_numbers: Optional[List[int]] = None, append: bool = True) -> str:
    """
    ...
    if page_numbers is None:
        ...
        for page_num in page_numbers:
            if 1 <= page_num <= len(pages):
                ...
                print(f"Warning: Page {page_num} out of range (total pages: {len(pages)})")
...
def extract_pages_with_content(input_file: str, output_file: str, search_content: str, append: bool = True) -> str:
    """
    ...
    print(f"Found {len(matching_pages)} pages containing the search content (pages: {matching_page_numbers})")
