...
class RentRollLocatorAgent:
    ...
    def _create_agent(self) -> AssistantAgent:
        ...
        system_prompt_path = os.path.join(
            os.path.dirname(__file__), "../prompts/rentroll_locator_system.md"
        )
    ...
    async def process_file(self, text_file_name: str, output_file_name: str) -> str:
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", suffix="_numbered.txt", delete=False
        ) as temp_file:
            numbered_file = temp_file.name

        logger.info("Adding line numbers to input file...")
        ...
        user_prompt_path = os.path.join(
            os.path.dirname(__file__), "../prompts/rentroll_locator_user.md"
        )
...
Analyze this page content and determine if it contains rent roll data. Respond ONLY with the required JSON format."""
            ...
            try:
                response = await self.agent.on_messages(
                    [page_message], cancellation_token=None
                )
                ...
                if response.chat_message and hasattr(response.chat_message, "content"):
                    ...
                    if decision == "YES":
                        ...
                        append_result = append_page_to_output(
                            output_file_name, page_content, page_num
                        )
                        pages_with_rentroll += 1
                        logger.info(
                            f"✅ Page {page_num} contains rent roll data - EXTRACTED PROGRAMMATICALLY"
                        )
                        logger.info(f"   {append_result}")
                    else:
                        logger.info(
                            f"❌ Page {page_num} does not contain rent roll data - SKIPPED"
                        )
    ...
    def _parse_llm_decision(self, llm_response: str) -> str:
        ...
        try:
            ...
            if (
                '"detection_result": "YES"' in llm_response
                or '"detection_result":"YES"' in llm_response
            ):
                return "YES"
            elif (
                '"detection_result": "NO"' in llm_response
                or '"detection_result":"NO"' in llm_response
            ):
                return "NO"
