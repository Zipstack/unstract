...
class TSVToJSONConverter:
    ...
    async def convert(
        self, tsv_file: str, json_schema: dict[str, Any], output_file: str
    ) -> str:
    ...
    def _parse_tsv(self, tsv_file: str) -> list[dict[str, str]]:
        ...
        for line_num, line in enumerate(lines[1:], start=2):
            ...
            if self._is_simplified_sub_row(fields, len(headers)):
                ...
                row_dict = self._parse_simplified_sub_row(
                    fields, array_field_info, headers
                )
            else:
    ...
    def _get_array_field_info(self, headers: list[str]) -> dict[str, Any]:
        ...
        """
        array_info = {"positions": [], "field_names": [], "array_name": None}
    ...
    def _parse_simplified_sub_row(
        self, fields: list[str], array_field_info: dict[str, Any], headers: list[str]
    ) -> dict[str, str]:
    ...
    def _build_json_object(
        self, row: dict[str, str], schema: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def convert_phase2(
        self, phase1_json_file: str, output_file: str, user_schema: str = None
    ) -> str:
        ...
        with open(phase1_json_file, encoding="utf-8") as f:
    ...
    def _consolidate_array_fields(
        self, json_data: list[dict[str, Any]], expected_arrays: set = None
    ) -> list[dict[str, Any]]:
    ...
    def _is_sub_row(self, json_obj: dict[str, Any]) -> bool:
        ...
        all_non_array_null = all(
            value is None or value == "" for value in non_array_values
        )
    ...
    def _merge_sub_row_into_parent(
        self, parent_obj: dict[str, Any], sub_row: dict[str, Any]
    ) -> None:
    ...
    def _generate_normalised_tsv(
        self, json_data: list[dict[str, Any]], output_file: str
    ) -> None:
        ...
        with open(output_file, "w", encoding="utf-8", newline="") as f:
            ...
            for obj in json_data:
                ...
                if array_data:
                    ...
                    max_items = (
                        max(len(items) for items in array_data.values())
                        if array_data
                        else 1
                    )
                    ...
                    for i in range(max_items):
                        ...
                        for array_name in sorted(array_fields.keys()):
                            for subfield in sorted(array_fields[array_name]):
                                value = ""
                                if array_name in array_data and i < len(
                                    array_data[array_name]
                                ):
                                    ...
                                    if isinstance(item, dict):
                                        ...
                                        value = (
                                            "" if item_value is None else str(item_value)
                                        )
...
async def convert_tsv_to_json(
    tsv_file: str, json_schema: dict[str, Any], output_file: str
) -> str:
...
async def convert_phase2_json(
    phase1_json_file: str, output_file: str, user_schema_file: str = None
) -> str:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print(
            "Usage: python tsv_to_json_converter.py <tsv_file> <field_mapping_file> <output_json_file>"
        )
        sys.exit(1)
