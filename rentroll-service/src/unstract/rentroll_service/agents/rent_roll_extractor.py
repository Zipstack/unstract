...
class RentRollExtractorAgent:
    ...
    def _create_agent(self) -> AssistantAgent:
        ...
        system_prompt_path = os.path.join(
            ...
            "../prompts/rentroll_extractor_system.md"
        )
    ...
    async def extract_data(self, extracted_text_file: str, field_mapping_file: str, output_tsv_file: str) -> str:
        """
        ...
        if not filtered_mapping:
            raise ValueError("No valid field mappings found. All fields are mapped to null.")
        ...
        if len(rent_roll_text) <= max_chunk_size:
            ...
            tsv_content = await self._extract_single_chunk(rent_roll_text, filtered_mapping)
        else:
            # Process in multiple chunks
            tsv_content = await self._extract_multiple_chunks(rent_roll_text, filtered_mapping, max_chunk_size)
    ...
    def _filter_array_parent_fields(self, field_mapping: Dict[str, str]) -> Dict[str, str]:
        """
        Filter out parent array fields when array element fields exist.
    ...
    def _generate_array_row_note(self, array_fields: List[str], field_mapping: Dict[str, str]) -> str:
        """
        ...
        for i, field_name in enumerate(array_fields, start=2):
            ...
            note_lines.append(f"- Column {i}: The value from document column `{document_column}` (mapped to `{field_name}`)")
    ...
    async def _extract_single_chunk(self, rent_roll_text: str, field_mapping: Dict[str, str]) -> str:
        """Extract data from a single text chunk."""
        # Load user prompt template
        user_prompt_path = os.path.join(
            os.path.dirname(__file__), 
            "../prompts/rentroll_extractor_user.md"
        )
    ...
    async def _extract_multiple_chunks(self, rent_roll_text: str, field_mapping: Dict[str, str], max_chunk_size: int) -> str:
    ...
    def _extract_tsv_from_response(self, response_content: str) -> str:
        ...
        for line in lines:
            ...
            if (line.startswith('line_nos\t') or 
                line.startswith('line_ref\t') or
                ('line_nos' in line and '\t' in line)):
                in_tsv_section = True
        ...
        if '\t' in response_content and ('0x' in response_content or 'line_nos' in response_content):
    ...
    def tsv_array_tab_normalizer(self, tsv_file_path: str) -> str:
        ...
        empty_tabs_needed = first_array_position - 1  # -1 because first column is line_nos
        ...
        for line_num, line in enumerate(lines[1:], start=2):
            ...
            if self._is_simplified_array_row(fields, len(headers)):
                ...
                normalized_line = self._normalize_simplified_array_row(fields, empty_tabs_needed)
    ...
    def _normalize_simplified_array_row(self, fields: List[str], empty_tabs_needed: int) -> str:
        """
