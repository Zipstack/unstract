...
def _run_workflow_api(
    ...
    **kwargs: dict[str, Any],
    ...
    try:
        ...
        # rather than reaching for a barrier directly, so both fan-out call
        # sites share one dispatch point. The header tasks + callback carry the
        # fairness slot for downstream routing.
        # Hoist FairnessKey to a local so the fan-out path and the
        ...
        # Surface the error to the UI + reconcile file counters so a failed run
        # reads "N failed" not "N in progress"; the wrapped update inside the
        # helper guarantees a status-update failure can't mask `e` or skip the
        # re-raise below (see record_pg_orchestration_failure).
        error_logger = None
        try:
