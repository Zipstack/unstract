...
class WorkerWorkflowExecutionService:
    ...
    def execute_workflow_for_file(
        ...
        try:
            ...
            # CRITICAL FIX: Pass file_data by reference so computed hash is preserved
            ...
            # LOG: Verify that computed hash is available after workflow execution
            computed_hash = file_data.get("file_hash", "")
            logger.info(
                f"After workflow execution, file_data contains hash: '{computed_hash}' for file {file_name}"
            )
            ...
            if success:
                ...
                try:
                    ...
                    if tool_results:
                        ...
                        # CRITICAL FIX: Extract computed file hash from tool results and update file_data
                        # This ensures the computed hash is available for file history creation
                        computed_file_hash = None
                        if isinstance(tool_results, dict):
                            # Try to extract hash from tool results metadata or output
                            if "metadata" in tool_results and isinstance(
                                tool_results["metadata"], dict
                            ):
                                computed_file_hash = tool_results["metadata"].get(
                                    "file_hash"
                                )
                            elif "output" in tool_results and isinstance(
                                tool_results["output"], dict
                            ):
                                computed_file_hash = tool_results["output"].get(
                                    "file_hash"
                                )

                        if computed_file_hash:
                            file_data["file_hash"] = computed_file_hash
                            logger.info(
                                f"HASH PROPAGATION: Updated file_data with computed hash from tool results: '{computed_file_hash}' for file {file_name}"
                            )
                        else:
                            logger.warning(
                                f"HASH PROPAGATION: No file_hash found in tool results for file {file_name}"
                            )
                    else:
                        logger.warning(
