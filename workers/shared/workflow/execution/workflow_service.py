...
class WorkerWorkflowExecutionService(WorkflowExecutionService):
    ...
    def __init__(
        ...
        Args:
            ...
            is_api: Whether this is an API workflow (affects file history behavior)
        """
    ...
    def _create_file_batches(
        ...
        if self.use_file_history:
            ...
            if len(files_to_process) != len(source_files):
                ...
                # For API workflows with file history enabled, if all files are cached,
                # we still need to process them to return the cached results
                if self.is_api and len(files_to_process) == 0:
                    logger.info(
                        "API workflow with all files cached - processing to return cached results"
                    )
                    files_to_process = (
                        source_files  # Process all files to get cached results
                    )
    ...
    def _check_file_history_batch(
        ...
        For API workflows:
        - When use_file_history=False: Process all files normally (don't skip)
        - When use_file_history=True: Return cached results from file history
        ...
        """
        # Handle empty source_files for all workflows
        if not source_files:
            return source_files

        # API workflows have special file history behavior
        if self.is_api:
            if not self.use_file_history:
                # For API workflows, when use_file_history=False, process all files normally
                return source_files
            # When use_file_history=True for API, continue to check history and return cached results
        else:
            # Regular workflows: use standard file history behavior
            if not self.use_file_history:
                return source_files
