...
function PromptCard({
  ...
  const handleRun = () => {
    ...
    handleRunApiRequest(docId)
      ...
      .catch((err) => {
        ...
        setAlertDetails(
          handleException(`Failed to generate output for ${docId}`)
        );
      })
      .finally(() => {
        if (isSimplePromptStudio) {
  ...
  const handleRunApiRequest = async (docId) => {
    ...
    if (!isSimplePromptStudio) {
      ...
      url = "http://localhost:8000/simple-prompt-studio/fetch";
    }
  ...
  const handleOutputApiRequest = async (isOutput) => {
    let url;
    if (isSimplePromptStudio) {
      url = `http://localhost:8000/simple-prompt-studio/prompt-output?tool_id=${details?.tool_id}&prompt_id=${promptDetails?.prompt_id}`;
    } else {
