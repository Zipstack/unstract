...
def create_llm_adapter(
    ...
    # Validate SDK1 path exists
    if not SDK1_ADAPTERS.exists():
        result["errors"].append(f"SDK1 adapters directory not found at: {SDK1_ADAPTERS}")
        return result

    llm_dir = SDK1_ADAPTERS / "llm1"
    static_dir = llm_dir / "static"

    if not llm_dir.exists():
        result["errors"].append(f"LLM adapters directory not found at: {llm_dir}")
        return result
    ...
    class_name = f"{class_base}LLMAdapter"
    ...
    if use_existing_param_class:
        ...
        param_class = f"{class_base}LLMParameters"
    else:
        param_class = "AnyscaleLLMParameters"

    # Create adapter file
    adapter_content = LLM_ADAPTER_TEMPLATE.format(
    ...
    adapter_file = llm_dir / f"{provider.lower()}.py"
    ...
    # Create JSON schema
    schema = json.loads(json.dumps(LLM_SCHEMA_TEMPLATE))
    schema["title"] = f"{display_name} LLM"
    schema["properties"]["adapter_name"]["description"] = (
        f"Provide a unique name for this adapter instance. Example: {provider.lower()}-llm-1"
    )
    ...
    # Generate parameter class stub if requested
    if add_param_class:
...
def main():
    parser = argparse.ArgumentParser(
        description="Initialize a new LLM adapter for unstract/sdk1"
    )
    parser.add_argument(
        "--provider",
        required=True,
    ...
    parser.add_argument(
        "--use-param-class",
        help="Use an existing parameter class (e.g., 'OpenAILLMParameters')"
    )
    parser.add_argument(
        "--logo-url",
    ...
    print(f"Creating LLM adapter for: {args.name}")
    ...
    if args.dry_run:
        ...
        print(f"  - {SDK1_ADAPTERS}/llm1/{args.provider.lower()}.py")
        print(f"  - {SDK1_ADAPTERS}/llm1/static/{args.provider.lower()}.json")
