# Callback Worker Dockerfile
# Use a specific version of Python slim image
FROM python:3.12.9-slim AS base

ARG VERSION=dev
LABEL maintainer="Zipstack Inc." \
    description="Callback Worker Container" \
    version="${VERSION}"
...
EXPOSE ${HEALTH_PORT:-8083}

# Default command - can be overridden
CMD ["/app/workers/callback/.venv/bin/python", "-m", "worker"]
