...
class AutomergingRetriever(BaseRetriever):
    ...
    def retrieve(self) -> set[str]:
        ...
        """
        try:
            logger.info(f"Retrieving chunks for {self.doc_id} using AutomergingRetriever.")
        ...
        for key in grouped:
            grouped[key].sort(
                key=lambda n: (
                    ...
                    if hasattr(n.node, 'metadata') else 0
                )
            )
            ...
            while i < len(nodes):
                ...
                merged_node = NodeWithScore(
                    node=type(current_node.node)(
                        ...
                        metadata=current_node.node.metadata if hasattr(current_node.node, 'metadata') else {}
                    ),
                    score=merged_score
                )
