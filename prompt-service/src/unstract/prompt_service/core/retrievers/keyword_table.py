...
class KeywordTableRetriever(BaseRetriever):
    ...
    def retrieve(self) -> set[str]:
        ...
        try:
            logger.info(
                f"Retrieving chunks for {self.doc_id} using KeywordTableRetriever."
            )
            ...
            base_retriever = vector_store_index.as_retriever(
                ...
                * 3,  # Get more candidates for keyword filtering
            ...
            sorted_nodes = sorted(
                scored_nodes.items(), key=lambda x: x[1][1], reverse=True
            )[: self.top_k]
            ...
            logger.info(
                f"Successfully retrieved {len(chunks)} chunks using keyword table."
            )
            return chunks
