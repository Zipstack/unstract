...
class QueryRouterRetriever(BaseRetriever):
    ...
    def __init__(
        self,
        *args,
        retriever_descriptions: Dict[str, str] = None,
        **kwargs
    ):
    ...
    def retrieve(self) -> set[str]:
        ...
        try:
            ...
            keyword_retriever = BM25Retriever(index=vector_index, similarity_top_k=self.top_k)
            ...
            logger.info(f"Successfully retrieved {len(context)} chunks using Router Retriever.")
            return context
