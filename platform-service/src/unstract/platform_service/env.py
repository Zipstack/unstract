...
class Env:
    ...
    DB_HOST = EnvManager.get_required_setting(
        "DB_HOST", os.environ.get("PG_BE_HOST")
    )
    DB_PORT = int(
        EnvManager.get_required_setting(
            "DB_PORT", os.environ.get("PG_BE_PORT", "5432")
        )
    )
    DB_USER = EnvManager.get_required_setting(
        "DB_USER", os.environ.get("PG_BE_USERNAME")
    )
    DB_PASSWORD = EnvManager.get_required_setting(
        "DB_PASSWORD", os.environ.get("PG_BE_PASSWORD")
    )
    DB_NAME = EnvManager.get_required_setting(
        "DB_NAME", os.environ.get("PG_BE_DATABASE")
    )
